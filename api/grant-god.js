...
//        body (JSON) : { code: "<合言葉>" }
// 動作 : 合言葉 (環境変数 GOD_GRANT_CODE) が一致したら、ログイン中の LINE ユーザーに
//        user_tags へ 'god' タグを付与する (冪等: 既にあれば何もしない)。
//        app.js 側は TAG_FEATURE_GRANTS.god で god-max (mode.godBatch) を解放する。
...
//        503 { error:'not_configured' }    … GOD_GRANT_CODE 未設定
//
// 設計メモ:
//   - plan='god' に更新する案は entitlements.plan の CHECK 制約に 'god' が無く
...
module.exports = async function handler(req, res) {
  ...
  const expected = process.env.GOD_GRANT_CODE;
  if (!expected || !expected.trim()) {
    console.error('[api/grant-god] GOD_GRANT_CODE not configured');
    res.statusCode = 503;
  ...
  try {
    ...
    if (e instanceof VerificationError) {
      console.warn('[api/grant-god] auth failed', { code: e.code });
      res.statusCode = e.httpStatus;
      return res.json({ error: e.code });
    }
    console.error('[api/grant-god] unexpected auth error:', e);
    res.statusCode = 500;
  ...
  if (!safeEqual(code, expected.trim())) {
    console.warn('[api/grant-god] wrong code attempt');
    res.statusCode = 403;
  ...
  try {
    ...
    console.error('[api/grant-god] supabase init failed:', e.message);
    res.statusCode = 500;
    return res.json({ error: 'internal_error' });
  }
  ...
  const { error } = await supabase
    .from('user_tags')
    .upsert(
      { line_user_id: auth.lineUserId, tag: GOD_TAG },
      { onConflict: 'line_user_id,tag', ignoreDuplicates: true }
    );
  if (error) {
    console.error('[api/grant-god] db error:', error.message);
    res.statusCode = 500;
    return res.json({ error: 'db_error' });
  }

  console.log('[api/grant-god] god tag granted');
  res.statusCode = 200;
